    ...
    def loss_sum_train(self, epochs, print_freq=10, val_freq=1):
        ...
        for epoch in range(epochs):
            ...
            for i, (z, label) in enumerate(self.train_loader):
                ...
                loss = for_loss * self.aided_weight + rev_loss
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.flow.parameters(), 1.)
                self.optimizer.step()
                ...
                if i%print_freq == 0:
                    print(f'iter {i} : for_loss = {for_loss_meter.avg * self.aided_weight}, rev_loss = {rev_loss_meter.avg}')
            print(f"*epoch {epoch}: for_loss = {for_loss_meter.avg * self.aided_weight}, rev_loss = {rev_loss_meter.avg}")
            if i%val_freq ==0:
                with torch.no_grad():
