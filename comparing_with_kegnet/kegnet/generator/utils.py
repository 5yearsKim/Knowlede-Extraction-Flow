...
from KEflow.model.utils import prepare_flow 
...
def sample_noises_flow(size):
    noise = torch.randn(size)
    # scale = torch.rand(size[0]).view(-1, 1, 1, 1) 
    scale = 1.
    return noise * scale
...
def sample_flow_data(dataset, num_data, generators, device):
    ...
    for path in generators:
        generator = prepare_flow("NICE", nc, 10)
        generator = generator.to(device)
        # load checkpoint
        state_dict = torch.load(path)
        generator.load_state_dict(state_dict["flow_state"])
        generator.eval()
        gen_models.append(generator)

    ny = 10
    z_size = (nc, 32, 32)
    noises = sample_noises_flow(size=(num_data, *z_size))
    ...
    for idx, generator in enumerate(gen_models):
        ...
        for z, y in loader:
            ...
            y = y.to(device)
            img, _ = generator(z, y, reverse=True)
            # print(img)
            l1.append(normalize_images(img.detach()))
