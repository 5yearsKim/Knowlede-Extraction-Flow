...
class ExtractorTrainer:
    ...
    def train(self, epochs, print_freq=10, val_freq=1, spread_s=0.04, gravity_s=0, bn_s=1. ,label_smoothe=0.):
    ...
    def train_step(self, x, label, loss_meter, spread_s, gravity_s, bn_s, smoothing):
        ...
        confidence = torch.log(confidence + 0.01)
        if smoothing != 0.:
        ...
        loss = ll_loss + grav_loss + bn_loss
        if torch.isfinite(ldj_loss):
            loss += ldj_loss
